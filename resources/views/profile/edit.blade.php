...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="dark">
    <head>
        ...
        <title>Editar Perfil - TurisApp</title>
        ...
        <div class="relative min-h-screen flex flex-col">
            ...
            <main class="flex-grow py-12 px-4 sm:px-6 lg:px-8">
                <div class="max-w-3xl mx-auto">
                    ...
                    <div class="bg-white dark:bg-dark-bg-secondary shadow overflow-hidden sm:rounded-lg p-6">
                         <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data">
                             ...
                             <div class="space-y-4">
                                 <div>
                                     ...
                                     <input type="text" name="phone" id="phone" value="{{ old('phone', $user->profile->phone ?? '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-dark-bg-secondary dark:border-gray-600 dark:text-white">
                                     @error('phone')
                                  ...
                                  <div>
                                     ...
                                     <textarea name="bio" id="bio" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-dark-bg-secondary dark:border-gray-600 dark:text-white">{{ old('bio', $user->profile->bio ?? '') }}</textarea>
                                     @error('bio')
                                         <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                     @enderror
                                 </div>
                                  ...
                                  <div>
                                     <label for="skills" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Destinos Favoritos (separados por coma)</label>
                                     <input type="text" name="skills" id="skills" value="{{ old('skills', is_array($user->profile->skills ?? null) ? implode(', ', $user->profile->skills) : '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-dark-bg-secondary dark:border-gray-600 dark:text-white">
                                     @error('skills')
                                         <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                     @enderror
                                 </div>
                                 ...
                                 <div>
                                     <label for="avatar" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Foto de perfil</label>
                                      <div class="mt-1 flex items-center space-x-4">
                                          {{-- Mostrar la foto de perfil actual o un placeholder --}}
                                          <img class="h-12 w-12 rounded-full object-cover" src="{{ $user->profile->avatar ? asset('storage/' . $user->profile->avatar) : asset('images/default_avatar.png') }}" alt="Foto de perfil actual">
                                           <input type="file" name="avatar" id="avatar" class="block w-full text-sm text-gray-500 dark:text-gray-400
                    ...
                    {{-- Opcional: Enlace para volver al perfil --}}
                    <div class="mt-8 text-center">
                        <a href="{{ route('profile.index') }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-500 text-sm font-medium">&larr; Volver al perfil</a>
                    </div>
